...
// made floating point autowall better ;)
...
bool Autowall::VectortoVectorVisible(Vector src, Vector point, C_BaseEntity* pEnt)
{
	//mask_solid
	C_Trace TraceInit;
...
void UTIL_ClipTraceToPlayers(const Vector& vecAbsStart, const Vector& vecAbsEnd, unsigned int mask, ITraceFilter* filter, C_Trace* tr) // sig is most likely out of date
{
...
bool HandleBulletPenetration(WeaponInfo_t *wpn_data, FireBulletData &data, bool extracheck = false, Vector point = Vector(0, 0, 0), C_BaseEntity* pEnt = nullptr)
{
	...
	if (extracheck)
		if (!Autowall::get().VectortoVectorVisible(trace_exit.end, point, pEnt))
			return false;
	thickness *= thickness;
	thickness *= v34;
	thickness /= 24.0f;
