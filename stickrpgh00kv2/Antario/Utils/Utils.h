...
public:
	...
	{
		/// Get module address
		const unsigned int moduleAddress = reinterpret_cast<unsigned int>(GetModuleHandle(module_name));
		...
		for (unsigned int c = 0; c < moduleInfo.SizeOfImage; c += 1)
		{
			/// does it match?
			if (IsCorrectMask(reinterpret_cast<unsigned char*>(moduleAddress + c), mask, mask_string))
				return (moduleAddress + c);
		}
